...
export class ComfyComponent extends BaseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public addClass(tableName: string): boolean {
    return this.targetTableIsLinkable(tableName);
  }

  private targetTableIsLinkable(tableName: string): boolean {
    return !(cdmTypes as any).default['Derived Elements'].includes(tableName.toUpperCase());
  }
