...
export class Loopback extends DataConnection {
  ...
  createScanRequest(settingsComponentForm) {
    return this.scanRequestService.create({body: {
      dataSourceConfig: {
  ...
  testConnection(settingsComponentForm: any): Observable<ConnectionResultWithTables> {
    return this.createScanRequest(settingsComponentForm)
    .pipe(map((r) => {
      return {
        canConnect: true,
        tablesToScan: [
