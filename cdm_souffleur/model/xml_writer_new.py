...
def get_sql_data(mapping_items):
    ...
    for index, row in mapping_data.iteritems():
        all_fields += [{x: dic[x] for x in required_fields} for dic in row]
...
def get_xml(json):
    ...
    for source_table in source_tables:
        ...
        for target_table in target_tables:
            target_table_data = mapping_items.loc[
                (mapping_items['source_table']
                 == source_table) & (mapping_items['target_table']
                                     == target_table)]
            option = target_table_data.get('option').fillna('').tolist()
            lookup = target_table_data.get('lookup').fillna('').tolist()
            mapping = target_table_data.get('mapping').fillna('').tolist()
